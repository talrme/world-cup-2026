...
Match Details can show a "Chance to advance" card for upcoming knockout matches that have two known teams and ESPN odds. The data is refreshed by `scripts/update_odds.py`, which reads each match's ESPN event id, fetches the ESPN event summary JSON, and writes `data/match-odds.json` / `match-odds.js`.

The displayed percentage links to the ESPN match page used as the source. ESPN currently exposes a 90-minute market with a possible draw, not a direct "to advance" market, so the site estimates advancement by converting each team's American moneyline odds into raw implied probability and normalizing the two team probabilities against each other. The expanded "Show math" section keeps the source, sportsbook, draw odds, and calculation visible.

This is an estimate from listed odds, not betting advice. Completed matches, placeholder/TBD matches, and matches without two real teams do not show the card.
...
- `match-odds.js`: generated browser odds data with sourced ESPN chance-to-advance estimates.
...
- `data/match-odds.json`: generated source odds data from ESPN event summaries.
...
- `scripts/update_odds.py`: ESPN odds updater for upcoming knockout chance-to-advance estimates.
...
- `scripts/refresh_all.py`: runs schedule refresh, ESPN bracket refresh, player stats refresh, ESPN odds refresh, highlight refresh, then optional AI insight refresh.
