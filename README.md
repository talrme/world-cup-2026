...
- Future matches with real team names are populated and then refreshed only when their source data/prompt changes or they become weekly stale. Placeholder knockout matches such as `Winner Group A` are skipped until real teams appear.
...
- `data/ai-insights.json`: source AI insight blurbs generated by Gemini.
...
- `scripts/update_ai_insights.py`: optional Gemini AI insight updater.
- `scripts/refresh_all.py`: runs schedule refresh, player stats refresh, highlight refresh, then optional AI insight refresh.
